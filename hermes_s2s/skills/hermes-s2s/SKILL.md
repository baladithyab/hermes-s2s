...
That's expected in 0.1.0. Realtime backends are stubs in this release — track progress at https://github.com/codeseys/hermes-s2s. Use `cascaded` or `s2s-server` in the meantime.
...
- **Hermes built-in voice mode is still active** — `hermes-s2s` adds providers; it does not replace `tools/transcription_tools.py` and `tools/tts_tool.py`. If a user has `stt.provider: groq` set in `voice.stt.provider` (Hermes built-in) AND `s2s.pipeline.stt.provider: moonshine` (this plugin), the built-in wins for built-in voice mode. Wire the Discord/CLI hooks (planned, 0.2.0) to actually route through this plugin.
